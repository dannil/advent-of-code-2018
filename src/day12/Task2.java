...
public class Task2 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        long numberOfGenerations = 50000000000L;

        int previousSum = 0;
        int previousDifference = 0;

        for (long i = 1; i <= numberOfGenerations; i++) {
            ...
            int sum = getSum(state, leftMostPotPosition);

            int difference = sum - previousSum;
            if (difference == previousDifference) {
                long finalScore = sum + (numberOfGenerations - i) * previousDifference;
